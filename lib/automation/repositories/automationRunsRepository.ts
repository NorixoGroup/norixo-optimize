...
export async function getAutomationWorkspaceControl(client: AutomationClient, workspaceId: string): Promise<AutomationWorkspaceControl | null> {
  ...
  return data == null ? null : { workspaceId: data.workspace_id, backlinksEnabled: data.backlinks_enabled, backlinkAutonomyEnabled: data.backlink_autonomy_enabled, backlinkOutreachScheduleApplyEnabled: data.backlink_outreach_schedule_apply_enabled, dryRunOnly: data.dry_run_only, disabledReason: data.disabled_reason };
}
