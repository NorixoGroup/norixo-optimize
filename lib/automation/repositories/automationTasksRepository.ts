...
/** Dedicated RPC hardcodes the five autonomy kinds; callers cannot widen reclaim scope. */
export async function reclaimExpiredBacklinkAutonomyTasks(client: BacklinkRepositoryClient, input: ReclaimExpiredAutomationTasksInput): Promise<AutomationTask[]> { const operation = "reclaimExpiredBacklinkAutonomyTasks"; const { data, error } = await client.rpc("reclaim_expired_backlink_autonomy_tasks", { p_workspace_id: input.workspaceId, p_run_id: input.runId, p_reclaimed_at: input.reclaimedAt, p_limit: input.limit }); if (error != null) throw normalizeBacklinkRepositoryError(operation, error); if (!Array.isArray(data)) throw new BacklinkRepositoryError({ code: "DATABASE", operation, message: "The database returned an invalid automation task result." }); return data.map(mapAutomationTask); }
