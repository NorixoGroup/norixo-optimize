import type { AutomationWorkspaceControl, CreateAutomationTaskInput } from "./types";
...
import { enterPromotedOpportunityContactResolution } from "./backlink-promotion-resolution-entry";
...
export async function runBacklinkAutonomyScheduler(
  ...
  const empty = (outcome: BacklinkAutonomySchedulerResult["outcome"]): BacklinkAutonomySchedulerResult => ({ outcome, workspacesScanned: 0, promotionsScanned: 0, proposed: [], created: 0, existing: 0, skipped: [] });
  ...
  const proposed: CreateAutomationTaskInput[] = [], skipped: string[] = []; let promotionsScanned = 0, created = 0, existing = 0;
  for (const workspace of controls) {
    ...
    for (const promotion of promotions) {
      promotionsScanned += 1;
      const result = await enterPromotedOpportunityContactResolution(deps, { promotion, control, mode, scheduledAt: input.scheduledAt });
  ...
  return { outcome: mode === "preview" ? "preview" : "applied", workspacesScanned: controls.length, promotionsScanned, proposed, created, existing, skipped };
}
