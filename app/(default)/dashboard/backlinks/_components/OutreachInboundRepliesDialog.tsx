...
type Props = {
  ...
  aiSubmittingMessageId: string | null;
  aiProposalMessageId: string | null;
  aiProposal: { reply: string; tone: string; language: string; warnings: string[] } | null;
  aiError: string | null;
  onClose(): void;
  onRequestAiProposal(messageId: string): void;
...
export default function OutreachInboundRepliesDialog({ open, outreachLabel, outreachStatus, outreachLastResponseType, outreachClosedAt, outreachStopReason, outreachNextFollowUpAt, replies, loading, loadError, submittingMessageId, classificationError, classificationSuccess, pendingConfirmation, aiSubmittingMessageId, aiProposalMessageId, aiProposal, aiError, onClose, onRequestAiProposal, onRequestClassification, onConfirmClassification, onCancelClassification }: Props) {
  if (!open) return null;
