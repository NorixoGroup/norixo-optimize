...
export async function POST(request: NextRequest) {
  ...
  const releaseHeldEntitlement = async (failureCode: string) => {
    if (!entitlementOperationKey || entitlementFinalized || auditPersisted) {
      return;
    }
  ...
  try {
    ...
    const quota = !billingAdminBypass
      ? await canCreateAudit(workspace.id, client)
      : null;
    ...
    const entitlementTargetRef = normalizeSourceUrl(body.url) ?? body.url.trim();
    const entitlementReservation = await reserveAuditEntitlement({
      workspaceId: workspace.id,
      userId: user.id,
      operationKey: entitlementOperationKey,
      targetKind: "source_url",
      targetRef: entitlementTargetRef,
      billingAdminBypass,
    ...
    if (isUnreliableAirbnbExtraction(extracted)) {
      console.warn("[api/listings][target-extraction-unreliable-airbnb]", {
    ...
    const vrboExtractionReliability =
      evaluateVrboExtractionReliability(extracted);

    if (isUnreliableVrboExtraction(extracted)) {
    ...
    if (isUnreliableAgodaExtraction(extracted)) {
      console.warn("[api/listings][target-extraction-unreliable-agoda]", {
        platform: extracted.platform ?? null,
    ...
    const expediaExtractionReliability =
      evaluateExpediaExtractionReliability(extracted);

    if (isUnreliableExpediaExtraction(extracted)) {
      console.warn(
        "[api/listings][target-extraction-unreliable-expedia]",
        {
          platform: extracted.platform ?? null,
          titleLength:
            expediaExtractionReliability.titleLength,
          descriptionLength:
            expediaExtractionReliability.descriptionLength,
          photoCount:
            expediaExtractionReliability.photoCount,
          amenityCount:
            expediaExtractionReliability.amenityCount,
          reasons:
            expediaExtractionReliability.reasons,
        },
      );
    ...
    const finalizeEntitlementResult = await finalizeAuditEntitlement({
      ...
      userId: user.id,
      usageSource: "api_listings_create",
      sourceUrl: extracted.url ?? body.url ?? null,
    });
        ...
        if (occupancyObservation != null) {
          try {
            ...
            if (
              ...
              console.warn(
                "[INTELLIGENCE_V2_OCCUPANCY_LISTING_CONTRIBUTION_FAILED]",
              );
            }
          }
        }
    ...
    await releaseHeldEntitlement("route_error");
