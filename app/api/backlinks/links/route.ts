...
import { createLink, listLinks, type CreateLinkInput } from "@/lib/backlinks/services/linkService";
...
function responseForError(error: unknown) {
  const code =
    typeof error === "object" && error != null && "code" in error && typeof error.code === "string"
      ? error.code
      : null;
  const message = error instanceof Error ? error.message : "Impossible de traiter les liens.";
  ...
  console.error("[backlinks/links] request failed", error);
  return NextResponse.json({ error: "Impossible de traiter les liens." }, { status: 500 });
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json(await listLinks(context.client, context.workspace.id));
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    return NextResponse.json(
      await createLink(context.client, context.workspace.id, context.user.id, body as CreateLinkInput),
      { status: 201 },
    );
  } catch (error) {
    return responseForError(error);
