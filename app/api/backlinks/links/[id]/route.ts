...
import { getLink, updateLink, type UpdateLinkInput } from "@/lib/backlinks/services/linkService";
...
function responseForError(error: unknown) {
  const code =
    typeof error === "object" && error != null && "code" in error && typeof error.code === "string"
      ? error.code
      : null;
  const message = error instanceof Error ? error.message : "Impossible de traiter le lien.";
  ...
  console.error("[backlinks/link] request failed", error);
  return NextResponse.json({ error: "Impossible de traiter le lien." }, { status: 500 });
}
...
export async function GET(
  ...
  try {
    return NextResponse.json(await getLink(requestContext.client, requestContext.workspace.id, id));
  } catch (error) {
...
export async function PATCH(
  ...
  try {
    return NextResponse.json(
      await updateLink(requestContext.client, requestContext.workspace.id, id, body as UpdateLinkInput),
    );
  } catch (error) {
    return responseForError(error);
  }
