...
async function main() {
  ...
  let contact: { id: string; domain_id: string; contact_status: string; email_normalized: string | null } = {
    id: "contact",
    domain_id: "domain",
  ...
  const service = sendBacklinkOutreachEmail({
    ...
    reserveAttempt: async (_workspaceId, input) => {
      ...
      if (storedAttempt != null) return { attempt: storedAttempt, disposition: "existing" as const };
      ...
      return { attempt: storedAttempt, disposition: "created" as const };
    },
