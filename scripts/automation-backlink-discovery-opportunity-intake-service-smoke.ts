...
function dependencies(value: BacklinkDiscoveryPreviewCandidate, lifecycleStatus = "active", existing = false) {
  const calls = { asset: 0, resolver: 0 };
...
async function main(): Promise<void> {
  ...
  assert(created.calls.asset === 1 && created.calls.resolver === 1 && createdResult.domainDisposition === "created" && createdResult.qualificationStatus === "Needs Review", "Eligible created intake.");
  ...
  assert(historical.calls.asset === 0 && historical.calls.resolver === 0, "Historical output must stop before mutations.");
  ...
  assert(review.calls.asset === 0 && review.calls.resolver === 0, "Review-only must stop before mutations.");
  ...
  assert(inactive.calls.resolver === 0, "Inactive asset must not resolve.");
