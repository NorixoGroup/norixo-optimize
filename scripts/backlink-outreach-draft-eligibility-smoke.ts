...
async function main() {
  ...
  const result = await getBacklinkOutreachDraftEligibilityForMembership({
    ...
    getOpportunity: async () => ({ id: "opportunity", domain_id: "domain-a", asset_id: "asset", target_page_url: "https://example.com/page" }),
    listContactsByDomain: async (_workspace, domainId) => {
      ...
      return [
        ...
        { id: "verified-form", contact_key: "CT-000005", full_name: null, role_title: "Editorial contact", contact_status: "verified", email_normalized: null, linkedin_url: null, contact_form_url: "https://example.com/verified-contact" },
    ...
    listCurrentVerifiedContactFormEvidenceContactIds: async () => new Set(["verified-form"]),
  }, { workspaceId: "workspace", campaignId: "campaign", opportunityId: "opportunity" });
  assert(result.contacts.length === 5 && result.domainId === "domain-a", "Expected all contacts for the domain.");
  ...
  assert(unverified.eligibleChannels.includes("email") && !unverified.eligibleChannels.includes("contact_form"), "Stored contact_form_url alone must not make contact-form eligible.");
  const verifiedForm = result.contacts[2];
  assert(verifiedForm.eligibleChannels.includes("contact_form"), "Verified form evidence must enable contact-form eligibility.");
  assert(result.contacts[3].eligibleChannels.length === 0 && result.contacts[3].unavailableReasons.includes("CONTACT_DO_NOT_CONTACT"), "Do-not-contact must be blocked.");
  assert(result.contacts[4].eligibleChannels.length === 0 && result.contacts[4].unavailableReasons.includes("CONTACT_ARCHIVED"), "Archived contacts must be blocked.");
  const selfExcluded = await getBacklinkOutreachDraftEligibilityForMembership({ getMembership: async () => ({ campaign_id: "campaign", opportunity_id: "opportunity" }), getOpportunity: async () => ({ id: "opportunity", domain_id: "domain-a", asset_id: "asset", target_page_url: "https://example.com/page" }), listContactsByDomain: async () => [{ id: "verified", contact_key: "CT-000001", full_name: "Verified", role_title: null, contact_status: "verified", email_normalized: "v@example.com", linkedin_url: null, contact_form_url: null }], listOutreachByOpportunity: async () => [{ id: "self", contact_id: "verified", channel: "email", status: "ready" }, { id: "other", contact_id: "verified", channel: "linkedin", status: "ready" }] }, { workspaceId: "workspace", campaignId: "campaign", opportunityId: "opportunity", excludeOutreachId: "self" });
