...
set attempt_kind = 'initial'
where attempt_kind is null;

alter table public.backlink_outreach_attempts
  alter column attempt_kind set not null,
  add constraint backlink_outreach_attempts_attempt_kind_check
    check (attempt_kind in ('initial', 'follow_up'));

comment on column public.backlink_outreach_attempts.attempt_kind is
  'Initial sends and scheduled follow-up reservations are distinct attempt kinds. Existing attempts are initial.';

create or replace function public.reserve_backlink_outreach_follow_up_attempt(
  p_workspace_id uuid,
  p_outreach_id uuid,
...
begin
  if p_workspace_id is null or p_outreach_id is null then
    raise exception 'FOLLOW_UP_OUTREACH_NOT_ACTIVE';
  end if;
  ...
  if p_reserved_at is null then
    raise exception 'FOLLOW_UP_NOT_DUE';
  end if;
  ...
  if found then
    if existing_attempt.outreach_id <> p_outreach_id
      or existing_attempt.attempt_kind <> 'follow_up' then
      raise exception 'FOLLOW_UP_IDEMPOTENCY_CONFLICT';
    end if;
    return query select
      'existing',
      existing_attempt.id,
      existing_attempt.outreach_id,
      existing_attempt.status,
      existing_attempt.attempt_kind,
      existing_attempt.requested_at;
    return;
  end if;
  ...
  for update;
  if not found or outreach.status <> 'active' then
    raise exception 'FOLLOW_UP_OUTREACH_NOT_ACTIVE';
  end if;
  if outreach.channel <> 'email' then
    raise exception 'FOLLOW_UP_CHANNEL_NOT_SUPPORTED';
  end if;
  if outreach.next_follow_up_at is null then
    raise exception 'FOLLOW_UP_NOT_SCHEDULED';
  end if;
  if outreach.next_follow_up_at > p_reserved_at then
    raise exception 'FOLLOW_UP_NOT_DUE';
  end if;
  if outreach.current_attempt >= outreach.max_attempts then
    raise exception 'FOLLOW_UP_ATTEMPT_LIMIT_REACHED';
  end if;

  -- Recheck after locking Outreach so concurrent requests with the same key are idempotent.
  select * into existing_attempt
  from public.backlink_outreach_attempts as attempt
  where attempt.workspace_id = p_workspace_id
    and attempt.idempotency_key = normalized_key;
  if found then
    if existing_attempt.outreach_id <> outreach.id
      or existing_attempt.attempt_kind <> 'follow_up' then
      raise exception 'FOLLOW_UP_IDEMPOTENCY_CONFLICT';
    end if;
    return query select
      'existing', existing_attempt.id, existing_attempt.outreach_id,
      existing_attempt.status, existing_attempt.attempt_kind, existing_attempt.requested_at;
    return;
  end if;
  ...
  if not found
    ...
    raise exception 'FOLLOW_UP_CONTACT_UNAVAILABLE';
  end if;
  ...
  if found then
    if open_attempt.status = 'unknown' then
      raise exception 'FOLLOW_UP_ATTEMPT_UNRESOLVED';
    end if;
  ...
  for update;
  if found then
    raise exception 'FOLLOW_UP_INBOUND_REPLY_STOPPED';
  end if;
  ...
  if not found then
    ...
    if found
      ...
      and existing_attempt.attempt_kind = 'follow_up' then
      return query select
        'existing', existing_attempt.id, existing_attempt.outreach_id,
        existing_attempt.status, existing_attempt.attempt_kind, existing_attempt.requested_at;
      return;
  ...
  return query select
    'reserved', reserved_attempt.id, reserved_attempt.outreach_id,
    reserved_attempt.status, reserved_attempt.attempt_kind, reserved_attempt.requested_at;
end;
$$;

revoke all on function public.reserve_backlink_outreach_follow_up_attempt(uuid, uuid, text, text, timestamptz) from public, anon, authenticated;
grant execute on function public.reserve_backlink_outreach_follow_up_attempt(uuid, uuid, text, text, timestamptz) to service_role;

comment on function public.reserve_backlink_outreach_follow_up_attempt(uuid, uuid, text, text, timestamptz) is
  'Atomically reserves one due email follow-up Attempt, consumes next_follow_up_at, and never sends through a provider.';
