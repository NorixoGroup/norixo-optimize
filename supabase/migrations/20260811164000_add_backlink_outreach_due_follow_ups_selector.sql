...
create function public.list_backlink_outreach_due_follow_ups(
  p_workspace_id uuid,
  p_now timestamptz,
...
begin
  if p_workspace_id is null
    ...
    raise exception 'FOLLOW_UP_DUE_SELECTOR_INVALID';
  end if;
  ...
  where outreach.workspace_id = p_workspace_id
    ...
    and outreach.next_follow_up_at is not null
    and outreach.response_deadline_at is null
    and outreach.next_follow_up_at <= p_now
  ...
  order by outreach.next_follow_up_at asc, outreach.id asc
  limit p_limit;
end;
$$;

revoke all on function public.list_backlink_outreach_due_follow_ups(uuid, timestamptz, integer) from public, anon, authenticated;
grant execute on function public.list_backlink_outreach_due_follow_ups(uuid, timestamptz, integer) to service_role;

comment on function public.list_backlink_outreach_due_follow_ups(uuid, timestamptz, integer) is
  'Read-only workspace-scoped selector for due follow-up outreaches. Returns only minimal identifiers and schedule state; it is not the authority for reservation, which must revalidate atomically.';
