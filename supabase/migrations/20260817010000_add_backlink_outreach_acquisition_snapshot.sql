...
alter table public.backlink_outreach
  add constraint backlink_outreach_success_acquisition_snapshot_check
  check (
    (
      success_link_id is null
      and success_link_status is null
      and success_verified_at is null
      and success_source_url is null
      and success_target_url is null
    )
    or (
      success_link_id is not null
      and success_link_status = 'active'
      and success_verified_at is not null
      and char_length(trim(coalesce(success_source_url, ''))) > 0
      and char_length(trim(coalesce(success_target_url, ''))) > 0
    )
  );
