...
if [[ -z "${SCENARIO_NAME}" ]]; then
  ...
  echo "Usage: bash marketing-agent/tools/run-publication-qa.sh <scenario-name> --item=<item-id>" >&2
  exit 1
fi
...
OUTPUT_FILE="${ITEM_DIR}/publication-qa-report.md"
...
if [[ -f "${OUTPUT_FILE}" ]]; then
  echo "Error: publication QA report already exists: ${OUTPUT_FILE}" >&2
  exit 1
fi

publication_request_status="FAIL"
...
if [[ -f "${PUBLICATION_REQUEST_FILE}" ]]; then
  publication_request_status="PASS"
fi
...
if [[ "${publication_request_status}" == "PASS" ]]; then
  required_fields_ok="YES"

  for field_name in "Platform" "Locale" "Content" "Image" "Video" "Publication Type" "Schedule" "Provider" "Status"; do
  ...
  content_status="$(read_section_value "Content")"
  image_status="$(read_section_value "Image")"
  video_status="$(read_section_value "Video")"

  if [[ "${required_fields_status}" == "PASS" && "${content_status}" == "READY" && "${image_status}" == "READY" && "${video_status}" == "READY" ]]; then
    overall_status="PASS"
  fi
fi
