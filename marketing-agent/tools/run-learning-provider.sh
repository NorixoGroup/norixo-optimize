...
LEARNING_PROVIDER_SCRIPT="${MARKETING_AGENT_DIR}/providers/learning/mock/provider.sh"
...
if [[ -z "${SCENARIO_NAME}" ]]; then
  ...
  echo "Usage: bash marketing-agent/tools/run-learning-provider.sh <scenario-name> --item=<item-id>" >&2
  exit 1
fi
...
OUTPUT_FILE="${ITEM_DIR}/learning-provider-report.md"
...
if [[ ! -f "${LEARNING_QA_REPORT_FILE}" ]]; then
  echo "Error: learning QA report not found: ${LEARNING_QA_REPORT_FILE}" >&2
  exit 1
fi

if [[ ! -f "${LEARNING_PROVIDER_SCRIPT}" ]]; then
  echo "Error: mock learning provider not found: ${LEARNING_PROVIDER_SCRIPT}" >&2
  exit 1
fi

if [[ -f "${OUTPUT_FILE}" ]]; then
  echo "Error: learning provider report already exists: ${OUTPUT_FILE}" >&2
  exit 1
fi

qa_overall_status="$(awk '/^Overall$/ { getline; getline; print; exit }' "${LEARNING_QA_REPORT_FILE}")"

if [[ "${qa_overall_status}" != "PASS" ]]; then
  echo "Error: learning QA is not PASS." >&2
  exit 1
fi

provider_output="$(bash "${LEARNING_PROVIDER_SCRIPT}" "${LEARNING_INPUT_FILE}")"
...
learning_status="$(printf '%s\n' "${provider_output}" | awk -F': ' '/^Learning:/ { print $2; exit }')"
decision_status="$(printf '%s\n' "${provider_output}" | awk -F': ' '/^Decision:/ { print $2; exit }')"
