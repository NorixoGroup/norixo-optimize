...
VIDEO_PROVIDER_SCRIPT="${MARKETING_AGENT_DIR}/providers/videos/mock/provider.sh"
...
if [[ -z "${SCENARIO_NAME}" ]]; then
  ...
  echo "Usage: bash marketing-agent/tools/run-video-provider.sh <scenario-name> --item=<item-id>" >&2
  exit 1
fi
...
OUTPUT_FILE="${ITEM_DIR}/video-provider-report.md"
...
if [[ ! -f "${VIDEO_QA_REPORT_FILE}" ]]; then
  echo "Error: video QA report not found: ${VIDEO_QA_REPORT_FILE}" >&2
  exit 1
fi

if [[ ! -f "${VIDEO_PROVIDER_SCRIPT}" ]]; then
  echo "Error: mock video provider not found: ${VIDEO_PROVIDER_SCRIPT}" >&2
  exit 1
fi

if [[ -f "${OUTPUT_FILE}" ]]; then
  echo "Error: video provider report already exists: ${OUTPUT_FILE}" >&2
  exit 1
fi

qa_overall_status="$(awk '/^Overall$/ { getline; getline; print; exit }' "${VIDEO_QA_REPORT_FILE}")"

if [[ "${qa_overall_status}" != "PASS" ]]; then
  echo "Error: video QA is not PASS." >&2
  exit 1
fi

provider_output="$(bash "${VIDEO_PROVIDER_SCRIPT}" "${VIDEO_STORYBOARD_FILE}")"
...
video_path="$(printf '%s\n' "${provider_output}" | awk -F': ' '/^Video Path:/ { print $2; exit }')"
